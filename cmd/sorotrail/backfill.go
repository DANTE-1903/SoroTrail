...
`)
	...
	fs.DurationVar(&f.progressInterval, "progress-interval", 0, "emit periodic progress to stderr (e.g. 30s, 1m); 0 disables")
...
func runBackfill(args []string) error {
	...
	if f.progressInterval > 0 {
		prog = NewProgress("backfill", f.progressInterval)
	}
